...
counter = 0
for i in range(100):
    sixdign = randint(1, 999999)
    num = str(sixdign)[3:]
    ru = randint(1, 10)
    counter += 1
    boole = True if randint(0,1) == 1 else False
    azs = AZS(id=counter, sixdign=sixdign, ru=ru, num=num, hostname='AZS-'+str(ru)+'-'+str(num)+'-CRYPTO', user_added=randint(1,2), address='DIFFERENT_address-'+str(sixdign), dzo=randint(1,2), azs_type=randint(1,3), active=boole, mss_ip='10.1.1.'+str(randint(1,254)), just_added=True)
